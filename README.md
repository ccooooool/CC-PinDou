...
将任意图片转换为拼豆（Perler Beads / 融合珠）制作图纸的 Web 工具。支持转换模式、像素模式、以及绘制模式三种工作模式。内置 5 个国内主流拼豆品牌的色号映射，采用 OKLab 感知均匀颜色空间进行最近色匹配，可导出带图例的高清 PNG/JPG 制作图纸和材料清单。
...
| 模式 | 适用场景 | 核心能力 |
|------|----------|----------|
| **转换模式** | 照片、插画等任意图片 | AI 背景移除 → 颜色简化 → OKLab 色号匹配 → 拼豆网格 |
| **像素模式** | 游戏素材、像素插画 | 自动像素块大小检测 + 对齐偏移微调 → 精确像素网格 |
| **绘制模式** | 从零原创图案 | 多图层画板 + 8 种工具 + 6 种对称 |
...
│   │   │   └── ui/                    # shadcn/ui 基础组件
│   │   ├── hooks/              # 自定义 Hooks
│   │   │   ├── useCanvasRenderer.ts   # Canvas 渲染（方块/圆形两模式）
