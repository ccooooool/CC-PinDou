...
export function usePatternGenerator({
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      if (generateAlgorithm === 'backend') {
        // 后端算法路径：调用 Python 后端生成
        const blob = await fetch(previewImage).then((r) => r.blob());
        const file = new File([blob], 'preview.png', { type: 'image/png' });
        const result = await generatePattern(file, {
          gridSize,
          colorSimplify,
          enhanceLines,
          colorMode,
        ...
        // 后端没有 BFS 连通合并，在前端补做
        const mergedGrid = engine.bfsMerge(result.grid_data, 25);
        ...
        const colorList = Array.from(finalColorMap.values()).sort((a, b) => b.count - a.count);
  ...
  }, [engine, previewImage, gridSize, colorSimplify, enhanceLines, colorMode, generateAlgorithm, onSuccess]);
